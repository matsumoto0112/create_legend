...
#include "assets/shaders/gpu_particle/enemy_move_particle.h"
#include "assets/shaders/gpu_particle/explosion.h"
...
std::shared_ptr<draw::particle::ParticleEmitter> CreateEnemyMoveParticle() {
  using directx::directx_helper::Failed;
  using namespace shaders::gpu_particle::enemy_move_particle;
  auto& device = game::GameDevice::GetInstance()->GetDevice();
  auto& resource = game::GameDevice::GetInstance()->GetResource();
...
std::shared_ptr<draw::particle::ParticleEmitter> CreateExplosionParticle() {
  using directx::directx_helper::Failed;
  using namespace shaders::gpu_particle::explosion;
  auto& device = game::GameDevice::GetInstance()->GetDevice();
  auto& resource = game::GameDevice::GetInstance()->GetResource();
  ...
  auto SetComputeShader = [&](directx::shader::ComputePipelineStateDesc& desc) {
    constexpr auto name =
        util::resource::resource_names::compute_shader::EXPLOSION;
    auto shader = resource.GetComputeShader().Get(name).get();
    desc.SetComputeShader(shader);
  };
