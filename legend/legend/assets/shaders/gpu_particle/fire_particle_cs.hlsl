#ifndef LEGEND_ASSETS_SHADERS_GPU_PARTICLE_FIRE_PARTICLE_CS_HLSL
#define LEGEND_ASSETS_SHADERS_GPU_PARTICLE_FIRE_PARTICLE_CS_HLSL
...
void ResetParticle(uint addr, uint index) {
  ...
  const float lifetime = RandomRange(0.5, 2.0, u, seed++);
  ...
  const float vy = RandomRange(-1.0, 1.0, u, seed++);
...
void UpdateParticle(uint addr) {
  ...
  float3 velocity = GetVelocity(addr);
  const float3 gravity = float3(0.0, -9.8, 0.0);
  velocity += gravity * g_global.delta_time;
  SetVelocity(addr, velocity);
...
#endif  //! LEGEND_ASSETS_SHADERS_GPU_PARTICLE_FIRE_PARTICLE_CS_HLSL
